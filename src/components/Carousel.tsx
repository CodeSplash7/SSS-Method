...
function useCarouselRenderErrors(
    ...
    const errors: [boolean, string][] = [
        ...
        [
            !itemsPerView ||
                !!(
                    options?.sm?.itemsPerView?.toString() &&
...
const CarouselList = ({
    ...
    useEffect(() => {
        ...
        setCarouselItems((prevItems) => {
            return prevItems?.map((item) => {
                ...
                const virtualIndex = calculateVirtualIndex(
                    newItem,
                    offset,
                    prevItems,
                );
                ...
                let fromX =
                    targetX -
                    directionIntValue *
                        (itemsPerView > 0 ? availableSpace : itemSpace);
                let toX = targetX;
                ...
                if (
                    ...
                    toX =
                        fromX - (itemsPerView > 0 ? availableSpace : itemSpace);
                    onComplete = () =>
                        animateItemX(
                            newItem.element.id,
                            null,
    ...
    useEffect(() => {
        ...
        const carouselItems: (CarouselItem | CarouselFillerItem)[] = [
            ...
            const element = itemListRef.current?.children[
                iIndex
            ] as HTMLElement;
        ...
        <div className="w-full h-full">
            ...
            >
                {React.Children.map(children, (child, i) => {
                    if (!React.isValidElement(child)) return null; // Ensure it's a ReactElement

                    return (
                        <div
                            key={child.key ?? i}
                            id={`${offset + itemWidth + itemHeight + itemsGap + itemsPerView}-item-${child.key ?? i}`}
                            style={{
                                width: itemWidth,
                                height: itemHeight,
                            }}
...
function calculateVirtualIndex(
    ...
    return (
        (((newItem.originalIndex + offset) % prevItems.length) +
            prevItems.length) %
        prevItems.length
    );
}
